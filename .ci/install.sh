...
docker exec -it --user root ndts /bin/sh -c 'apt-get -qq update; export DEBIAN_FRONTEND=noninteractive; apt-get install -y tango-db tango-common; sleep 10'
if [ $? -ne "0" ]
then
    exit -1
fi
...
if [ $2 = "2" ]; then
	...
	docker exec -it --user root ndts /bin/sh -c 'apt-get -qq update; export DEBIAN_FRONTEND=noninteractive; apt-get -qq install -y   python3-tango'
    else
